...
function NewsCard({ item, onClick }: NewsCardProps) {
  ...
  return (
    <article
      ...
      className="group relative bg-onyx/40 backdrop-blur-2xl rounded-3xl overflow-hidden border border-white/5 hover:border-rose/30 hover:bg-onyx/60 transition-all duration-500 cursor-pointer hover:shadow-[0_0_40px_-10px_rgba(244,63,94,0.3)] hover:-translate-y-2"
    >
      <div className="flex flex-col sm:flex-row">
        <div className="relative w-full sm:w-72 h-72 sm:h-56 overflow-hidden bg-onyx">
          {imageLoading && (
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="w-10 h-10 border-2 border-rose/20 border-t-rose rounded-full animate-spin"></div>
            </div>
          )}
          ...
          <div className="absolute bottom-4 left-4">
            <div className="inline-flex items-center gap-2 px-3 py-1 bg-rose/90 backdrop-blur-md rounded-full border border-white/10 shadow-lg">
        ...
        <div className="flex-1 p-8 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="flex items-center gap-3 px-6 py-2.5 bg-white/5 backdrop-blur-md rounded-full border border-white/10 group-hover:bg-rose group-hover:border-rose group-hover:shadow-[0_0_20px_rgba(244,63,94,0.4)] transition-all duration-500">
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-rose to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
    </article>
  );
}
